...
func (g *powLoginGuard) Render(w http.ResponseWriter, r *http.Request, lang string, scriptNonce string) template.HTML {
	...
	progressLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_progress"))
	attemptsLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_attempts"))
	elapsedLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_elapsed"))
	rateLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_rate"))
	readyLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_ready"))
	maskedLabel := template.JSEscapeString(g.translator.Text(lang, "pow_metric_masked"))
	noscript := template.HTMLEscapeString(g.translator.Text(lang, "pow_noscript"))

	return template.HTML(fmt.Sprintf(`
<div class="pow-box" data-pow-box>
  <div class="pow-title">%s</div>
  <div class="pow-description">%s</div>
  ...
  <div class="pow-progress" data-pow-progress>
    ...
    <div class="pow-progress-meta">
      <span class="pow-progress-percent" data-pow-progress-percent>0%%</span>
      <span class="pow-progress-stat" data-pow-attempts>%s: 0</span>
      <span class="pow-progress-stat" data-pow-elapsed>%s: 0.0s</span>
      <span class="pow-progress-stat" data-pow-rate>%s: 0 H/s</span>
    </div>
  </div>
...
(() => {
  ...
  const progressPercentNode = form.querySelector('[data-pow-progress-percent]');
  const attemptsNode = form.querySelector('[data-pow-attempts]');
  const elapsedNode = form.querySelector('[data-pow-elapsed]');
  const rateNode = form.querySelector('[data-pow-rate]');
  if (!submitButton || !tokenInput || !nonceInput || !difficultyInput || !statusNode || !progressTrack || !progressFill || !progressPercentNode || !attemptsNode || !elapsedNode || !rateNode) return;
  ...
  const progressLabel = "%s";
  const attemptsLabel = "%s";
  const elapsedLabel = "%s";
  const rateLabel = "%s";
  const readyLabel = "%s";
  const maskedLabel = "%s";
  ...
  function updateProgress(attempts, elapsedSeconds, solved) {
    ...
    let percent = 100;
    let attemptsText = maskedLabel;
    let elapsedText = maskedLabel;
    let rateText = maskedLabel;

    if (solved) {
      percent = 100;
      if (progressMode === "estimated") {
        attemptsText = attempts.toLocaleString();
        elapsedText = formatSeconds(elapsedSeconds);
        const finalRate = elapsedSeconds > 0 ? attempts / elapsedSeconds : 0;
        rateText = Math.round(finalRate).toLocaleString() + " H/s";
      }
    } else if (progressMode === "fake") {
      percent = fakeProgressPercent(elapsedSeconds);
    } else {
      percent = estimatedProgressPercent(attempts);
      attemptsText = attempts.toLocaleString();
      elapsedText = formatSeconds(elapsedSeconds);
      const liveRate = elapsedSeconds > 0 ? attempts / elapsedSeconds : 0;
      rateText = Math.round(liveRate).toLocaleString() + " H/s";
    }

    progressFill.style.width = percent.toFixed(1) + "%%";
    progressTrack.setAttribute("aria-valuenow", percent.toFixed(1));
    progressPercentNode.textContent = solved ? readyLabel : progressLabel + " " + percent.toFixed(1) + "%%";
    attemptsNode.textContent = attemptsLabel + ": " + attemptsText;
    elapsedNode.textContent = elapsedLabel + ": " + elapsedText;
    rateNode.textContent = rateLabel + ": " + rateText;
  }
  ...
  solve().catch(() => {
    statusNode.textContent = "%s";
		...
		template.HTMLEscapeString(g.translator.Text(lang, "pow_title")),
		template.HTMLEscapeString(fmt.Sprintf(g.translator.Text(lang, "pow_description"), difficulty)),
		template.HTMLEscapeString(g.translator.Text(lang, "pow_status_waiting")),
		template.HTMLEscapeString(g.translator.Text(lang, "pow_metric_attempts")),
		template.HTMLEscapeString(g.translator.Text(lang, "pow_metric_elapsed")),
		template.HTMLEscapeString(g.translator.Text(lang, "pow_metric_rate")),
